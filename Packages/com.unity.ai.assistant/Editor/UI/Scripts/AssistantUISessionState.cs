...
namespace Unity.AI.Assistant.UI.Editor.Scripts
{
    internal class AssistantUISessionState : ScriptableSingleton<AssistantUISessionState>
    {
        ...
        const string k_ExitPlanModeStateByCallId = k_Prefix + "ExitPlanModeStateByCallId";
        ...
        [Serializable]
        class ExitPlanModeStateCache
        {
            public List<ExitPlanModeStateEntry> entries = new();
        }
        ...
        public void SetExitPlanModeState(Guid callId, string conversationId, string planPath, string planContent, string title, bool expanded)
        {
            var key = callId.ToString();
            var cache = SessionStateJsonCache.Load(k_ExitPlanModeStateByCallId, () => new ExitPlanModeStateCache());
            ...
            {
                cache.entries.Add(new ExitPlanModeStateEntry
                {
                    callId = key,
            ...
            SessionStateJsonCache.Save(k_ExitPlanModeStateByCallId, cache, cache.entries.Count == 0);
        }

        public List<(Guid callId, string planPath, string planContent, string title, bool expanded)> GetExitPlanModeStatesForConversation(string conversationId)
        {
        ...
        public string GetExitPlanModeConversationId(Guid callId)
        {
            var key = callId.ToString();
            var cache = SessionStateJsonCache.Load(k_ExitPlanModeStateByCallId, () => new ExitPlanModeStateCache());
        ...
        public bool GetExitPlanModeExpanded(Guid callId)
        {
            var key = callId.ToString();
            var cache = SessionStateJsonCache.Load(k_ExitPlanModeStateByCallId, () => new ExitPlanModeStateCache());
            var entry = cache.entries.Find(e => e.callId == key);
        ...
        public void ClearExitPlanModeState(Guid callId)
        {
            var key = callId.ToString();
            var cache = SessionStateJsonCache.Load(k_ExitPlanModeStateByCallId, () => new ExitPlanModeStateCache());
