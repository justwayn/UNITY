...
namespace Unity.AI.Assistant.UI.Editor.Scripts.Components.ChatElements
{
    class AssistantWindowUiContainer : IToolUiContainer, IDisposable
    {
        ...
        {
            var entries = AssistantUISessionState.instance.GetExitPlanModeStatesForConversation(conversationId);
        ...
        void RegisterExitPlanModeInteraction(Guid callId, ExitPlanModeInteractionElement exitPlanMode, string conversationId)
        {
            ...
            if (string.IsNullOrEmpty(conversationId))
                conversationId = AssistantUISessionState.instance.GetExitPlanModeConversationId(callId);
        ...
        public void PushElement<TOutput>(ToolExecutionContext.CallInfo callInfo, IInteractionSource<TOutput> userInteraction)
        {
            ...
            if (userInteraction is ExitPlanModeInteractionElement exitPlanMode)
            {
                ...
                {
                    existingExitPlanMode.TaskCompletionSource.Task.ContinueWith(t =>
                    {
                        if (t.IsCanceled)
                            exitPlanMode.TaskCompletionSource.TrySetCanceled();
                        else if (t.IsFaulted)
                    ...
                    exitPlanMode.TaskCompletionSource.Task.ContinueWith(
                        _ => existingExitPlanMode.CancelInteraction(),
                        TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.NotOnRanToCompletion);
                    return;
                }

                RegisterExitPlanModeInteraction(callInfo.CallId, exitPlanMode, m_Context.Blackboard.ActiveConversationId.Value);
                return;
            }
        ...
        void CancelPendingInlineInteractions(string preserveConversationId = null)
        {
            ...
            {
                if (m_Context.PendingInlineInteractions.TryGetValue(key, out var view) && view is IInteractionSource<string> source)
                    sourcesToCancel.Add(source);
        ...
        sealed class PlanApprovalLifecycle : IDisposable
        {
            ...
            void OnExpandedStateChanged(bool isExpanded) =>
                AssistantUISessionState.instance.SetExitPlanModeState(m_CallId, ConversationId, m_Element.PlanPath, m_Element.PlanContent, m_Element.PlanTitle, isExpanded);

            void OnFooterCancel()
            {
                m_Owner.RemoveExitPlanModeEntry(m_CallId);
                m_Element.CancelInteraction();
            }

            void OnElementCompleted()
            {
                m_Owner.m_CompletedBeforeBridge[m_CallId] = m_Element.TaskCompletionSource.Task;
                m_Owner.RemoveExitPlanModeEntry(m_CallId);
